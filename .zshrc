...
alias jt="func() { local directory=$(echo '$(fd --color=always --type d ^ $@ | fzf --ansi --no-multi)') && local session=$(echo '${directory##*/}') && [ $(echo '$directory') ] && [ -d $(echo '$directory') ] && [ $(echo '$session') ] && tmux new -ds $(echo '$session') -c $(echo '$directory'); tmux switch -t $(echo '$session') || tmux attach -t $(echo '$session'); }; func"
