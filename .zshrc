...
alias adduser="func() { curl -X PUT --user mskar:$(echo '$(pass ${2:-ga/github/token})') -H 'Accept: application/vnd.github.v3+json' https://$(echo '${3:-git.generalassemb.ly}')/api/v3/orgs/py/memberships/$(echo '$1') -d '{\"role\":\"member\"}'; }; func"
...
alias ee="func() { local both=$(echo '$(exa --all --classify --color=always $@ | fzf --ansi )') && [ $(echo '$both') ] && echo $(echo '$both') | tr '\n' '\0' | xargs -0 open --; }; func"
...
alias j="func() { local directory=$(echo '$(fd --type d $@ | fzf --no-multi)') && [ $(echo '$directory') ] && [ -d $(echo '$directory') ] && cd $(echo '$directory'); }; func"
alias jt="func() { local directory=$(echo '$(fd --type d $@ | fzf --no-multi)') && [ $(echo '$directory') ] && [ -d $(echo '$directory') ] && cd $(echo '$directory') && local session=$(echo '${1:-${directory##*/}}') && [ $(echo '$session') ] && tmux switch -t $(echo '$session') || tmux new -ds $(echo '$session') && tmux switch -t $(echo '$session') || tmux attach -t $(echo '$session') ; }; func"
...
alias jjt="func() { local directory=$(echo '$(fasd -Rdl | fzf --delimiter=/ --no-multi --with-nth=4..)') && [ $(echo '$directory') ] && [ -d $(echo '$directory') ] && cd $(echo '$directory') && local session=$(echo '${1:-${directory##*/}}') && [ $(echo '$session') ] && tmux switch -t $(echo '$session') || tmux new -ds $(echo '$session') && tmux switch -t $(echo '$session') || tmux attach -t $(echo '$session'); }; func"
...
alias nfh="func() { local files=$(echo '$(fd --type f --hidden $@ | fzf)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 nvim --; }; func"
...
alias o="open"
alias od="func() { local directory=$(echo '$(fd --type d $@ | fzf)') && [ $(echo '$directory') ] && [ -d $(echo '$directory') ] && echo $(echo '$directory') | tr '\n' '\0' | xargs -0 open; }; func"
...
alias pgpua="pass git push -u --all"
...
alias pp="func() { local passwords=$(echo '$(pass | tail -n +2 | cut -c5- | sed "s+[└├│─]+ +g ; s+^[[:space:]]+/+ ; s+[[:space:]]++g" | tr "\n" " " | sed "s+[[:space:]]/+/+g" | tr " " "\n" | fzf --ansi --no-multi --no-preview)') && [ $(echo '$passwords') ] && echo $(echo '$passwords') | xargs pass $(echo '${@:--c}'); }; func"
alias pr="pass rm"
...
alias vz="v ~/.zshrc"
alias w="which"
alias ww="func() { local files=$(echo '$(fd -e js --type f $@ | fzf)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 webstorm; }; func"
