...
alias nn="func() { local files; files=$(echo '$(fasd -Rfl | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\\|/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$@ -- $files') | xargs nvim; }; func";
