...
alias a="git add"
alias aa="func() { local files; files=$(echo '$(git ls-files -dmo --exclude-standard | fzf --ansi -m --no-sort --preview="if [ \$(git ls-files --other --exclude-standard {} | sed s/\ //g) ]; then; git diff --color=always --no-index -- /dev/null {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\|/g | sed s/$/\|$/g || echo ^) --color=always; else; git diff --color=always {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\|/g | sed s/$/\|$/g || echo ^) --color=always; fi" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 git add $(echo '$@') --; }; func"
