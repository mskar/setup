...
alias ff="func() { local files; files=$(echo '$(git diff HEAD --name-status --relative --diff-filter=M $@ | fzf --ansi -m --no-sort --preview="git diff HEAD --color=always \$(echo {} | cut -c3-) | rg -e {q} -e ^ --color=always" --preview-window="70%" --reverse | cut -c3-)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 git checkout HEAD --; }; func"
alias fl='fasd -fl'
...
alias kk="func() {local untracked; untracked=$(echo '$(git clean -dfnx | sed s/^Would\ remove\ // | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | rg -e {q} -e ^ --color=always" --preview-window="70%" --reverse)') && [ $(echo '$untracked') ] && echo $(echo '$untracked') | tr '\n' '\0' | xargs -0 git clean -dfx; }; func";
...
alias ne="func() { local files; files=$(echo '$(n -u NONE -es "+pu =v:oldfiles" +%p +q! | sed s+$HOME+~+ | fzf -m --no-sort --preview="echo {} | sed s+~+\$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 nvim $(echo '$@') --; }; func";
alias nf="func() { local files; files=$(echo '$(fd --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 nvim $(echo '$@') --; }; func";
alias ng="func() { local file; file=$(echo '$(rg -l $@ | fzf --no-sort --preview="rg --color=always -n $@ {}" --preview-window="70%" --reverse)') && [ $(echo '$file') ] && echo $(echo '$file') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 nvim +$(echo '$(rg -n $@ $file | head -n 1 | cut -d: -f1)') --; }; func";
alias nh="n -c History" # this only works with -c, not --cmd
alias nl="func() { local files; files=$(echo '$(rg -l $@ | fzf -m --no-sort --preview="rg --color=always -n $@ {}" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 nvim --; }; func";
alias nn="func() { local files; files=$(echo '$(fasd -Rfl | sed s+$HOME+~+ | fzf -m --no-sort --preview="echo {} | sed s+~+\$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 nvim $(echo '$@') --; }; func";
alias no="n -c 'browse oldfiles'" # this only works with -c, not --cmd
alias nr="func() { local files; files=$(echo '$(rg -e "^> ~/" -e "^> /" ~/.viminfo | cut -c3- | fzf -m --no-sort --preview="echo {} | sed s+~+$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 nvim $(echo '$@') --; }; func";
...
alias of="func() { local files; files=$(echo '$(fd --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\\|/g | sed s/$/\\|$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 open; }; func";
alias oh="func() { local files; files=$(echo '$(fd -e html --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\\|/g | sed s/$/\\|$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 open; }; func";
...
alias or="func() { local files; files=$(echo '$(fd -e Rproj --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | xargs | sed s/\ /\\|/g | sed s/$/\\|$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 open; }; func";
alias oo="func() { local both; both=$(echo '$(fasd -Ral | fzf -m --no-sort --reverse)') && [ $(echo '$both') ] && echo $(echo '$both') | tr '\n' '\0' | xargs -0 open; }; func"
alias ow="func() { local files; files=$(echo '$(fd -e docx --type f ^ $@ | fzf -m --no-sort --preview="pandoc {} -t markdown | bat --style=numbers --color=always -l md | rg -e {q} -e ^ --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | tr '\n' '\0' | xargs -0 -0 open; }; func";
alias p="git push"
alias pc="pycharm"
alias pd="pycharm diff"
alias pp="func() { local files; files=$(echo '$(fd -e py --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | rg -e {q} -e ^ --color=alway" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 pycharm; }; func";
alias py="python"
...
# alias r="ranger"
alias r="git reset"
alias rr="func() { local files; files=$(echo '$(git diff --cached --name-status --relative $@ | fzf --ansi -m --no-sort --preview="git diff HEAD --color=always \$(echo {} | cut -c3-) | rg -e {q} -e ^ --color=always" --preview-window="70%" --reverse | cut -c3-)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 git reset HEAD --; }; func"
...
alias ve="func() { local files; files=$(echo '$(v -es --noplugin -u ~/.vimrc "+set nonumber" "+pu =v:oldfiles" +%p +q! | sed /^.$/d | fzf -m --no-sort --preview="echo {} | sed s+~+\$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 -o $EDITOR $(echo '$@') --; }; func";
alias vf="func() { local files; files=$(echo '$(fd --type f ^ $@ | fzf -m --no-sort --preview="bat --style=numbers --color=always {} | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | tr '\n' '\0' | xargs -0 -o $EDITOR; }; func";
alias vg="func() { local file; file=$(echo '$(rg -l $@ | fzf --no-sort --preview="rg --color=always -n $@ {}" --preview-window="70%" --reverse)') && [ $(echo '$file') ] && echo $(echo '$file') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 -o $EDITOR +$(echo '$(rg -n $@ $file | head -n 1 | cut -d: -f1)') --; }; func";
alias vh="v -c History" # this only works with -c, not --cmd
alias vl="func() { local files; files=$(echo '$(rg -l $@ | fzf -m --no-sort --preview="rg --color=always -n $@ {}" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 -o $EDITOR; }; func";
alias vo="n -c 'browse oldfiles'" # this only works with -c, not --cmd
alias vr="func() { local files; files=$(echo '$(rg -e "^> ~/" -e "^> /" ~/.viminfo | cut -c3- | fzf -m --no-sort --preview="echo {} | sed s+~+$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 -o $EDITOR $(echo '$@') --; }; func";
...
alias vv="func() { local files; files=$(echo '$(fasd -Rfl | sed s+$HOME+~+ | fzf -m --no-sort --preview="echo {} | sed s+~+\$HOME+ | xargs bat --style=numbers --color=always | grep -E \$([ {q} ] && echo {q} | sed s/\ //g | sed s/./\&\|/g | sed s/$/$/g || echo ^) --color=always" --preview-window="70%" --reverse)') && [ $(echo '$files') ] && echo $(echo '$files') | sed s+~+$HOME+ | tr '\n' '\0' | xargs -0 -o $EDITOR $(echo '$@') --; }; func";
