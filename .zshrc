...
alias tb="tig blame"
alias tc="func() { local directory=$(echo '${2:-${${1#*.*[:/]}%.*}}') && local session=$(echo '${3:-${directory##*/}}') && git clone $(echo '$1 $directory') && cd $(echo '$directory'); if ! tmux has-session -t $(echo '$session') &>/dev/null; then; tmux new -ds $(echo '$session') -c $(echo '$directory') && cd $(echo '$directory'); fi; if [ -n '$TMUX' ]; then; tmux switch -t $(echo '$session'); else; tmux attach -t $(echo '$session') -c $(echo '$directory') && cd $(echo '$directory'); fi; }; func" # clone and cd into repo then create or switch to or attach tmux session, ${parameter#pattern} removes pattern from the beginning, while ${parameter%pattern} removes pattern from the end
